...
interface OptimizationToolCardDisplayProps {
  data?: any
  readme: string
  onGoToFile?: (filePath: string) => void
  onSaveOptimizationToolSetup?: (configData: any) => Promise<void>
  allRepoFiles?: any[]
}

export default function OptimizationToolCardDisplay({
  data,
  readme,
  onGoToFile,
  onSaveOptimizationToolSetup,
  allRepoFiles,
}: OptimizationToolCardDisplayProps) {
  ...
  // Optimization tool setup state
  const [toolType, setToolType] = useState<"problem" | "optimizer">(data?.type || "problem")
  ...
  const [toolParameters, setToolParameters] = useState<Array<{ name: string; value: string }>>(
    data?.default_params
  ...
  const [toolKeywords, setToolKeywords] = useState<string[]>(data?.keywords || ["optimization"])
  ...
  const handleCompleteSetup = async () => {
    if (!entrypointFile || !onSaveOptimizationToolSetup) return
    ...
    try {
      ...
      const configData = {
        ...
        keywords: toolKeywords.length > 0 ? toolKeywords : ["optimization"],
      }
      ...
      // Save the config
      await onSaveOptimizationToolSetup(configData)
      ...
      toast({
        title: "Success",
        description: "Optimization tool setup completed successfully!",
      })
    } catch (err) {
      console.error("Error completing optimization tool setup:", err)
      toast({
        title: "Error",
        description: "Failed to complete optimization tool setup",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Optimization Tool Setup Dialog */}
      <Dialog open={showSetupDialog} onOpenChange={setShowSetupDialog}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Optimization Tool Setup</DialogTitle>
          </DialogHeader>
            ...
            >
              ...
              <div className={currentSetupStep === 1 ? "block" : "hidden"}>
                <div className="space-y-2 mt-4">
                  ...
                  <Select value={toolType} onValueChange={(value: "problem" | "optimizer") => setToolType(value)}>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {toolType === "problem"
                      ? "An optimization tool that defines a problem to be solved"
                      : "An optimization tool that implements an algorithm to solve problems"}
                  </p>
                </div>
              ...
              {currentSetupStep !== 1 && (
                <div className="flex justify-between items-center">
                  <div className="text-sm">{toolType === "problem" ? "Problem" : "Optimizer"}</div>
            ...
            >
              ...
              <div className={currentSetupStep === 2 ? "block" : "hidden"}>
                <div className="space-y-2 mt-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Select a Python file that contains your {toolType === "problem" ? "BaseProblem" : "BaseOptimizer"}{" "}
                    class
                  </p>
                </div>
                ...
                <div className="space-y-2 mt-4">
                  ...
                  <Input
                    id="entrypointClass"
                    placeholder={toolType === "problem" ? "MyProblem" : "MyOptimizer"}
                    value={entrypointClass}
                    onChange={(e) => setEntrypointClass(e.target.value)}
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    The name of the {toolType === "problem" ? "BaseProblem" : "BaseOptimizer"} class in the selected
                    file
                  </p>
                </div>
              ...
              {currentSetupStep !== 3 && currentSetupStep > 3 && (
                <div className="flex justify-between items-center">
                  <div className="text-sm">
                    {toolParameters.length > 0 ? (
                      <span>
                        {toolParameters.length} parameter{toolParameters.length !== 1 ? "s" : ""} defined
                      </span>
                    ) : (
                ...
                <div className="space-y-4 mt-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {toolKeywords.map((keyword, index) => (
                        <Badge key={index} variant="secondary" className="flex items-center gap-1">
                          {keyword}
                          <X
                            className="h-3 w-3 cursor-pointer"
                            onClick={() => {
                              const newKeywords = [...toolKeywords]
                              newKeywords.splice(index, 1)
                              setToolKeywords(newKeywords)
                            }}
                          />
                        </Badge>
                      ))}
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault()
                            if (newKeyword.trim() && !toolKeywords.includes(newKeyword.trim())) {
                              setToolKeywords([...toolKeywords, newKeyword.trim()])
                              setNewKeyword("")
                            }
                          }
                        }}
                    ...
                    <p className="text-xs text-muted-foreground">
                      Add keywords to help others find your optimization tool (e.g., optimization, quantum, scheduling)
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="text-sm">
                    <div className="flex gap-2">
                      ...
                      {toolKeywords.length > 0 && (
                        <span>
                          {toolKeywords.length} keyword{toolKeywords.length !== 1 ? "s" : ""}
                        </span>
                      )}
                      {arxivLinks.length === 0 && toolKeywords.length === 0 && (
