...
const OptimizationWorkflows = () => {
  ...
  useEffect(() => {
    fetchWorkflows()
  }, [searchQuery, sortBy, sortOrder, currentPage, activeTab])
  ...
  const fetchWorkflows = async () => {
    ...
    try {
      ...
      const endpoint = activeTab === 'personal'
        ? `${API}/api/playground/workflows/personal?${params}`
        : `${API}/api/playground/workflows/community?${params}`

      const response = await fetch(endpoint, {
        credentials: 'include' // Include cookies for authentication (needed for personal tab)
      })
      ...
      if (response.ok) {
        ...
        if (response.status === 401 && activeTab === 'personal') {
          toast({
            title: "Authentication Required",
            description: "Please log in to view your personal workflows.",
            variant: "destructive"
          })
          // Switch to community tab if not authenticated
...
interface WorkflowsContentProps {
  ...
  handleDeleteWorkflow?: (workflow: OptimizationWorkflow) => void
  tabType: 'community' | 'personal'
}
    ...
    <>
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSearch} className="space-y-4">
            <div className="flex gap-4">
              ...
              {tabType === 'personal' && handleDeleteWorkflow && workflows.length > 0 && (
                <DropdownMenu>
      ...
      <div className="mb-6 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {loading ? 'Loading...' : `${total} ${tabType} workflow${total !== 1 ? 's' : ''} found`}
        </p>
        <div className="flex items-center gap-2">
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <Button onClick={() => navigate('/qubots-playground')}>
              {tabType === 'personal' ? 'Create Your First Workflow' : 'Create a Workflow'}
            </Button>
          </CardContent>
        </Card>
      ) : (
