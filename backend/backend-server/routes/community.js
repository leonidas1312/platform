...
router.put("/comments/:id", auth, async (req, res) => {
  ...
  try {
    const comment = await knex("community_comments")
      .where({ id })
      .first()
    ...
    const [updatedComment] = await knex("community_comments")
      .where({ id })
      .update({
...
router.post("/posts/:id/like", auth, async (req, res) => {
  ...
  try {
    ...
    const existingLike = await knex("community_likes")
      .where({ post_id: id, username })
      .first()

    if (existingLike) {
      // Unlike the post
      await knex("community_likes")
        .where({ post_id: id, username })
        .del()
...
router.get("/posts/:id/likes", async (req, res) => {
  ...
  try {
    const likeCount = await knex("community_likes")
      .where({ post_id: id })
      .count("* as count")
      .first()
...
router.get("/posts/:id/liked", auth, async (req, res) => {
  ...
  try {
    const like = await knex("community_likes")
      .where({ post_id: id, username })
      .first()
